...
if(serveTableForm.ids)
{
	...
	if(l)
	{
		for(i=0;i<l;i++)
		{
			...
			{
			    	window.location = '<html:rewrite page="/technologySupportMaintStationAction.do"/>?id='+document.serveTableForm.ids[i].value +'&method=toPrepareUpdateRecord';
			    	return;
			}
		}
		if(l>0)
	...
	{
		window.location = '<html:rewrite page="/technologySupportMaintStationAction.do"/>?id='+document.serveTableForm.ids.value +'&method=toPrepareUpdateRecord'; 
	}
	else
	{
		alert("<bean:message key="technologySupport.role.alert"/>");
...
function Evenmore(obj,listname,td){	
	...
	if(comid!="" && comid!="%"){
	 ...
	 var url='<html:rewrite page="/technologySupportMaintStationAction.do"/>?method=toStorageIDList&comid='+comid;//跳转路径
