...
if(serveTableForm.ids)
{
	...
	if(l)
	{
		for(i=0;i<l;i++)
		{
			...
			//alert(document.serveTableForm.ids[i].value);
				window.location = '<html:rewrite page="/technologySupportTechnicalSupportAction.do"/>?id='+document.serveTableForm.ids[i].value +'&method=toPrepareUpdateRecord';
				return;
			}
		}
		if(l>0)
	...
	{
		window.location = '<html:rewrite page="/technologySupportTechnicalSupportAction.do"/>?id='+document.serveTableForm.ids.value +'&method=toPrepareUpdateRecord';
	}
	else
	{
		alert("<bean:message key="technologySupport.role.alert"/>");
