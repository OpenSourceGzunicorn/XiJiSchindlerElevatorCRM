...
if(serveTableForm.ids)
{
	...
	if(l)
	{
		for(i=0;i<l;i++)
		{
			if(document.serveTableForm.ids[i].checked == true)
			{
				...
				var elevatorType=document.getElementsByName("elevatorType")[i].value;
				var floor=document.getElementsByName("floor")[i].value;
				window.location = '<html:rewrite page="/maintenanceWorkingHoursAction.do"/>?id='+elevatorType+'&floor='+floor+'&method=toDisplayRecord';
				return;
			}
		}
		if(l >0)
	...
	{
		var elevatorType=document.getElementsByName("elevatorType")[0].value;
...
//增加
function addMethod(){
window.location = '<html:rewrite page="/maintenanceWorkingHoursAction.do"/>?method=toPrepareAddRecord';
}

//导入excel
function importMethod(){
window.location = '<html:rewrite page="/maintenanceWorkingHoursAction.do"/>?method=toPrepareImportRecord';
}
	...
	if(l)
	{
		for(i=0;i<l;i++)
		{
			...
			//alert(document.serveTableForm.ids[i].value);
			    var elevatorType=document.getElementsByName("elevatorType")[i].value;
				var floor=document.getElementsByName("floor")[i].value;
				window.location = '<html:rewrite page="/maintenanceWorkingHoursAction.do"/>?id='+elevatorType+'&floor='+floor+'&method=toPrepareUpdateRecord';
				return;
			}
		}
		if(l>0)
	...
	if(l)
	{
		for(i=0;i<l;i++)
		{
			if(document.serveTableForm.ids[i].checked == true)
			{
				...
				var elevatorType=document.getElementsByName("elevatorType")[i].value;
				var floor=document.getElementsByName("floor")[i].value;
				if(confirm("<bean:message key="javascript.role.deletecomfirm"/>"+elevatorType+" "+floor))
				{
					
					window.location = '<html:rewrite page="/maintenanceWorkingHoursAction.do"/>?id='+elevatorType+'&floor='+floor+'&method=toDeleteRecord';
				}
				return;
			}
		}
