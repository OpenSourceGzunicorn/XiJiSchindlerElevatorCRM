...
function CreateToolBar(){
  ...
  <logic:equal name="<%=SysConfig.TOOLBAR_RIGHT%>" property="nmbfimport" value="Y">
...
function importMethod(){
	window.location = '<html:rewrite page="/mbfImportAction.do"/>?method=toPrepareImportRecord';
}
...
function toDoMethod(index,method,params,alertMsg){
	if(index >= 0){	
		window.location = '<html:rewrite page="/mbfImportAction.do"/>?id='+getVal('ids',index)+'&method='+method+params;
...
function Evenmore(obj,listname){	
	...
	if(comid!="" && comid!="%"){
		 ...
		 var url='<html:rewrite page="/mbfImportAction.do"/>?method=toStorageIDList&comid='+comid;//跳转路径
