...
function returnMethod(){
  window.location = '<html:rewrite page="/outsideFootballElevatorSearchAction.do"/>?method=toSearchRecord';
}

//保存
function saveMethod(){
  document.outsideFootballElevatorForm.isreturn.value = "N";
  document.outsideFootballElevatorForm.submit();
}

//保存返回
function saveReturnMethod(){
  document.outsideFootballElevatorForm.isreturn.value = "Y";
  document.outsideFootballElevatorForm.submit();
}
...
function deleteFile(tablename,elevatorNo,value,td){
	  tbs=td;
	    if(confirm("确定删除吗")){
			...
			var uri = '<html:rewrite page="/outsideFootballElevatorAction.do"/>?method=toDeleteFileRecord';
			uri +='&filesid='+ value;
...
function downloadFile(name,tablename){
	var uri = '<html:rewrite page="/outsideFootballElevatorAction.do"/>?method=toDownloadFileRecord';
		uri +='&filesname='+ name;
		uri +='&folder=ElevatorArchivesInfo.file.upload.folder';
		uri +='&tablename='+tablename;
	window.location = uri;
