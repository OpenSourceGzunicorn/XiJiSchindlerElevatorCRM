...
if(serveTableForm.ids)
{
  ...
  if(l)
  {
    for(i=0;i<l;i++)
    {
      ...
      {
        window.location = '<html:rewrite page="/principalAction.do"/>?id='+document.serveTableForm.ids[i].value +'&method=toDisplayRecord';
        return;
      }
    }
    if(l >0)
  ...
  {
    window.location = '<html:rewrite page="/principalAction.do"/>?id='+document.serveTableForm.ids.value +'&method=toDisplayRecord';
  }
  else
  {
    alert("<bean:message key="javascript.role.alert2"/>");
...
//增加
function addMethod(){
window.location = '<html:rewrite page="/principalAction.do"/>?method=toPrepareAddRecord';
}
//修改
...
if(serveTableForm.ids)
{
  ...
  if(l)
  {
    for(i=0;i<l;i++)
    {
      ...
      //alert(document.serveTableForm.ids[i].value);
        window.location = '<html:rewrite page="/principalAction.do"/>?id='+document.serveTableForm.ids[i].value +'&method=toPrepareUpdateRecord';
        return;
      }
    }
    if(l >0)
  ...
  {
    window.location = '<html:rewrite page="/principalAction.do"/>?id='+document.serveTableForm.ids.value +'&method=toPrepareUpdateRecord';
  }
  else
  {
    alert("<bean:message key="javascript.role.alert1"/>");
...
if(serveTableForm.ids)
{
  ...
  if(l)
  {
    for(i=0;i<l;i++)
    {
      if(document.serveTableForm.ids[i].checked == true)
      {
        ...
        {
          window.location = '<html:rewrite page="/principalAction.do"/>?id='+document.serveTableForm.ids[i].value +'&method=toDeleteRecord';
        }
        return;
      }
    }
  ...
  else if(document.serveTableForm.ids.checked == true)
  {
    if(confirm("<bean:message key="javascript.role.deletecomfirm"/>"+document.serveTableForm.ids.value))
      {
        window.location = '<html:rewrite page="/principalAction.do"/>?id='+document.serveTableForm.ids.value +'&method=toDeleteRecord';
      }
  }
  else
  {
