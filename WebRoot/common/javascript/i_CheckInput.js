...
function CheckTel(a_tel)
{
	var l_strTemp,i
	var TEL
	var l_sResult,l_strSub
	TEL = a_tel;
	l_strTemp="0123456789-()#"
	for (i=0;i < TEL.length-1;i++){
		l_strSub = TEL.substring(i,i+1);
		l_sResult = l_strTemp.indexOf(l_strSub);
		if(l_sResult == -1)return false;
	}
	return true;
}
...
function Validate_Input()
{
	...
	for (i=0;i<l_intLen;i++)
	{
		...
		case "4":
		case "tel":
			if(!CheckTel(l_strValue)){
				l_obj.focus();
				return (l_strDesc + "不是合法的号码！");
			}
			break;
		default:
			break;
