...
function TANGER_OCX_SaveDoc()
{
	...
	try
	{
		...
		(
			...
			"saleElevatorContractGroundForm" //控件的智能提交功能可以允许同时提交选定的表单的所有数据.此处可使用id或者序号
	   ...
	   /* newwin = window.open("","_blank","left=200,top=200,width=400,height=300,status=0,toolbar=0,menubar=0,location=0,scrollbars=1,resizable=1",false);
		...
		newdoc = newwin.document;
		newdoc.open();
		//newdoc.write("<html><head><title>"+retHTML+"</title></head><body><center><hr>")
		newdoc.write(retHTML+"<hr>");*/
		//newdoc.write("<input type=button VALUE='关闭窗口' onclick='window.close()'>");
		//newdoc.write('</center></body></html>');*/
	}
...
function TANGER_OCX_SaveAsHTML()
{
	...
	//取工号后三位判断，以后三位为准将相邻正数个数为三个及以上且右减左数相差为1的数用A~B缩写形式显示，将（非数字字符，负数，0，正数）用，分隔
	function getServiceFloor_elevatorno(arraytoconvert){
   		//var floor=<%--floor --%>;
   		var floor=arraytoconvert.length;
   		var i=0;
   		var servicefloor="";
   		var floorarray=new Array();
   		var k=0;
   		var preelement=null;//记录上一次迭代的元素
   		
   		var firstcode="";//工号的前缀部分
   		for(;i<floor;i++){
   			var notNumChar=arraytoconvert[i].substring(arraytoconvert[i].length-3,arraytoconvert[i].length);
   			firstcode=arraytoconvert[i].substring(0,arraytoconvert[i].length-3);
   			if(notNumChar!=''){//过滤空字符串
		   		if(isNaN(notNumChar)){
		   			if(preelement==null){
		   		    	floorarray[k]=firstcode+notNumChar+",";
		   		    }else if(isNaN(preelement)){
		   		    	floorarray[k]=floorarray[k]+firstcode+notNumChar+",";
		   		    }else{
		   		    	k=k+1;
		   		    	floorarray[k]=firstcode+notNumChar+",";
		   		    }
		   		}
		   		else if(parseInt(notNumChar,10)<0){
		   			if(preelement==null){
		   		    	floorarray[k]=firstcode+notNumChar+",";
		   		    }else if(parseInt(preelement,10)<0){
		   		    	floorarray[k]=floorarray[k]+firstcode+notNumChar+",";
		   		    }else if(isNaN(preelement)
		   		    		||(!isNaN(preelement)&&parseInt(preelement,10)>0)){
		   		    	k=k+1;
		   		    	floorarray[k]=firstcode+notNumChar+",";
		   		    }
		   		}
		   			...
		   			if(preelement==null){
		   		        ...
		   		        if((parseInt(notNumChar,10)-parseInt(preelement,10))==1){
		   		    		floorarray[k]=floorarray[k]+firstcode+notNumChar+",";
		   		    	}else if((parseInt(notNumChar,10)-parseInt(preelement,10))>1){
		   		    		k=k+1;
		   		    		floorarray[k]=firstcode+notNumChar+",";
		   		    	}
		   		    }else if(isNaN(preelement)
   		    	...
   		    	preelement=notNumChar;
		   	}
   		}
   		var j=0;
   		for(;j<floorarray.length;j++){
   			var strarray=floorarray[j].split(",");
   			var firststring=strarray[0].substring(strarray[0].length-3,strarray[0].length);;
   			if(isNaN(firststring)){
   				servicefloor=servicefloor+floorarray[j];
   			}else if(parseInt(firststring,10)<0){
   				servicefloor=servicefloor+floorarray[j];
   			}else if(parseInt(firststring,10)==0){
   				servicefloor=servicefloor+floorarray[j];
   			}else if(parseInt(firststring,10)>0){
   				...
   				if(strarray.length>3){
   					temparray[0]=strarray[0];
   					temparray[1]=strarray[strarray.length-2];
   					floorarray[j]=temparray.join("～")+",";
   				}
   				servicefloor=servicefloor+floorarray[j];
   			}
   		}
   		servicefloor=servicefloor.substring(0,servicefloor.length-1);
   		
   		return servicefloor;
   }
   
   function initworknodisplay(){
   		//修改工号显示
   		if(document.getElementById("workno")){
   		var worknoarray=document.getElementById("workno").value.split(',');
   		document.getElementById("workno").value=getServiceFloor_elevatorno(worknoarray);
   		}
   }
   if(window.attachEvent){
	   window.attachEvent("onload",initworknodisplay);
	}else{//FireFox
		window.addEventListener("load",initworknodisplay);
	}
