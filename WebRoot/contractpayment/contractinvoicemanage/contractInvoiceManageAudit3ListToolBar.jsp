...
function CreateToolBar(){
  ...
  <%-- 是否有可写的权限--%>
  <logic:equal name="<%=SysConfig.TOOLBAR_RIGHT%>" property="ncontractinvoiceaudit3" value="Y"> 
...
function modifyMethod2(id,auditStatus){
	if(auditStatus == "N" || auditStatus=="X"){
		window.location = '<html:rewrite page="/contractInvoiceManageAudit3Action.do"/>?id='+id+'&method=toPrepareAuditRecord';
	}else{
		window.location = '<html:rewrite page="/contractInvoiceManageAudit3Action.do"/>?id='+id+'&method=toAuditDisplay';
	}
...
function toDoMethod(index,method,params,alertMsg){
	if(index >= 0){	
		window.location = '<html:rewrite page="/contractInvoiceManageAudit3Action.do"/>?id='+getVal('ids',index)+'&method='+method+params;
