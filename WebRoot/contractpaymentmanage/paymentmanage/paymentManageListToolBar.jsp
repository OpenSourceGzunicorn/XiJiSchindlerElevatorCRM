...
function viewMethod(){
	var index = getIndex();	
	toDoMethod(index,"toDisplayRecord","&returnMethod=toSearchRecord"+"&tcjnlNo="+getVal('tcjnlNo',index),"<bean:message key="javascript.role.alert2"/>");
}

//新建
function addMethod(){
	window.location = '<html:rewrite page="/paymentManageSearchAction.do"/>?method=toSearchNext';
}
...
function modifyMethod(){
	...
	toDoMethod(index,"toPrepareUpdateRecord","&tcjnlNo="+getVal('tcjnlNo',index),"<bean:message key="javascript.role.alert1"/>");
}
...
function deleteMethod(valstr){
	var index = getIndex();
	if(index >= 0){	
		var r2=getVal("r2",index);//流程状态
		var submitType=getVal("submitType",index);//提交状态	
		
		if(submitType == "N" || r2=="审批终止"){
...
function printMethod(){
	var index = getIndex();	
	if(index >= 0){	
		var status=getVal("r2",index);//提交状态		
		if(status && status != "审批通过"){
			alert("该记录未通过审核,不能打印通知单!"); 
			return;
		}
		window.open('<html:rewrite page="/paymentManageAction.do"/>?id='+getVal('ids',index)+'&method=toPrintRecord');
	}else{
		alert("请选择一条记录打印！");
	}
}
...
function toDoMethod(index,method,params,alertMsg){
	if(index >= 0){	
		window.location = '<html:rewrite page="/paymentManageAction.do"/>?id='+getVal('ids',index)+'&method='+method+params;
