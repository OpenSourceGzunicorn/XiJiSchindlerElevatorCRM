...
function CreateToolBar(){
  ...
  <%-- 是否有可写的权限--%>
  <logic:equal name="<%=SysConfig.TOOLBAR_RIGHT%>" property="nfreefittingcostsmanage" value="Y"> 
...
function returnMethod(){
  window.location = '<html:rewrite page="/freeFittingCostsManageSearchAction.do"/>?method=toSearchRecord';
}

//保存
function saveMethod(){
  if(verify()){
	  document.freeFittingCostsManageForm.fittings.value=rowsToJSONArray("prb","fittings");
	  document.freeFittingCostsManageForm.isreturn.value="N";
	  document.freeFittingCostsManageForm.submit();
  }
...
function saveReturnMethod(){
  ...
  if(verify()){
	  document.freeFittingCostsManageForm.fittings.value=rowsToJSONArray("prb","fittings");
	  document.freeFittingCostsManageForm.isreturn.value="Y";
      document.freeFittingCostsManageForm.submit();
    } 
}
...
function addElevators(tableId){
  ...
  if(returnarray!=null && returnarray.length>0){          
    ...
    toSetInputValue(returnarray,"freeFittingCostsManageForm");//向页面对应输入框赋值
  }    
...
function verify(){
	if(isNanTable() && checkColumnInput(freeFittingCostsManageForm) && checkRowInput("prb","prbT_0")){
		return true;
	}
	return false;
}
