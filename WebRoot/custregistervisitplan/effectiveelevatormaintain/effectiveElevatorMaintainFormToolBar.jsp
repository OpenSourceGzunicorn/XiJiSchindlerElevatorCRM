...
function returnMethod(){
  window.location = '<html:rewrite page="/effectiveElevatorSearchAction.do"/>?method=toSearchRecord';
}
...
if(cid!=null&&cid!=" "){
		...
		if(rod!=null&&rod!=" "){ */
			...
			//document.effectiveElevatorMaintainForm.contracts.value=rowsToJSONArray("dynamictable_0","contracts");
			if(checkColumnInput(effectiveElevatorMaintainForm)){
				document.effectiveElevatorMaintainForm.isreturn.value = "N";
		    	document.effectiveElevatorMaintainForm.submit();
		    }
		   /*  }else{
		    	alert("请添加丢梯客户合同明细！");
		    } */
	...
	if(cid!=null&&cid!=""){
		...
		/* var maintContractNo=document.getElementsByName("maintContractNo");
			if(maintContractNo.length>0){ */
				//document.effectiveElevatorMaintainForm.contracts.value=rowsToJSONArray("dynamictable_0","contracts");
				if(checkColumnInput(effectiveElevatorMaintainForm)){
					document.effectiveElevatorMaintainForm.isreturn.value = "Y";
			    	document.effectiveElevatorMaintainForm.submit();
			    }
		   /* 	}else{
		   		alert("请添加丢梯客户合同明细！");
...
function addElevators(tableId){
  ...
  paramstring+=contactPhone.value+"&wbBillnos="
