...
function CreateToolBar(){
  ...
  <%-- 是否有可写的权限--%>
  <logic:equal name="<%=SysConfig.TOOLBAR_RIGHT%>" property="neffectiveelevatorregisterhandle" value="Y"> 
...
function returnMethod(){
    window.location = '<html:rewrite page="/effectiveElevatorRegisterHandleAction.do"/>?method=toSearchRecord';
}
...
function saveMethod(){
	var handleResult=document.getElementsByName("handleResult");	
	  if(handleResult[0].value!=""){
		  document.effectiveElevatorRegisterHandleForm.isreturn.value = "N";
...
function saveReturnMethod(){
	var handleResult=document.getElementsByName("handleResult");
if(handleResult[0].value!=""){
	  document.effectiveElevatorRegisterHandleForm.isreturn.value = "Y";
      document.effectiveElevatorRegisterHandleForm.submit();
   }else
   {
	   var tile="<bean:message key='custreturnregistercheck.error.handleResult.required'/>";
