...
function searchMethod(){
	effectiveElevatorRegisterHandleForm.genReport.value = "";
	effectiveElevatorRegisterHandleForm.target = "_self";
	document.effectiveElevatorRegisterHandleForm.submit();
}
...
function toDoMethod(index,name,method,params,alertMsg){
	if(index >= 0){	
		window.location = '<html:rewrite page="/effectiveElevatorRegisterHandleAction.do"/>?id='+getVal(name,index)+'&method='+method+params;
...
//获取选中记录下标
function getIndex(){
	if(effectiveElevatorRegisterHandleForm.ids){
		var ids = effectiveElevatorRegisterHandleForm.ids;
		if(ids.length == null){
			if(ids.checked==true)
				return 0;
			else
...
function getVal(name,index){
	var obj = eval("effectiveElevatorRegisterHandleForm."+name);
	if(obj && obj.length){
		obj = obj[index];
	}
