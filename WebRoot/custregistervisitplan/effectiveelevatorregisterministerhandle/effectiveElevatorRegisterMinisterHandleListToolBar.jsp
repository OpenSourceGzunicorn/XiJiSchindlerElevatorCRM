...
function searchMethod(){
	effectiveElevatorRegisterMinisterHandleForm.genReport.value = "";
	effectiveElevatorRegisterMinisterHandleForm.target = "_self";
...
function toDoMethod(index,name,method,params,alertMsg){
	if(index >= 0){	
		window.location = '<html:rewrite page="/effectiveElevatorRegisterMinisterHandleAction.do"/>?id='+getVal(name,index)+'&method='+method+params;
...
//获取选中记录下标
function getIndex(){
	if(effectiveElevatorRegisterMinisterHandleForm.ids){
		var ids = effectiveElevatorRegisterMinisterHandleForm.ids;
		if(ids.length == null){
			if(ids.checked==true)
				return 0;
			else
...
function getVal(name,index){
	var obj = eval("effectiveElevatorRegisterMinisterHandleForm."+name);
	if(obj && obj.length){
		obj = obj[index];
	}
