...
function CreateToolBar(){
  ...
  <%-- 是否有可写的权限--%>
  <logic:equal name="<%=SysConfig.TOOLBAR_RIGHT%>" property="nlostelevatorregisterhandle" value="Y"> 
...
function returnMethod(){
    window.location = '<html:rewrite page="/lostElevatorRegisterHandleAction.do"/>?method=toSearchRecord';
}
...
function saveMethod(){
	var handleResult=document.getElementsByName("handleResult");	
	  if(handleResult[0].value!=""){
		  document.lostElevatorRegisterHandleForm.isreturn.value = "N";
...
function saveReturnMethod(){
	var handleResult=document.getElementsByName("handleResult");
if(handleResult[0].value!=""){
	  document.lostElevatorRegisterHandleForm.isreturn.value = "Y";
      document.lostElevatorRegisterHandleForm.submit();
   }else
   {
	   var tile="<bean:message key='custreturnregistercheck.error.handleResult.required'/>";
