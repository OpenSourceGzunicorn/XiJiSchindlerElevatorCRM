...
function searchMethod(){
	lostElevatorRegisterHandleForm.genReport.value = "";
	lostElevatorRegisterHandleForm.target = "_self";
	document.lostElevatorRegisterHandleForm.submit();
}
...
function toDoMethod(index,name,method,params,alertMsg){
	if(index >= 0){	
		window.location = '<html:rewrite page="/lostElevatorRegisterHandleAction.do"/>?id='+getVal(name,index)+'&method='+method+params;
...
//获取选中记录下标
function getIndex(){
	if(lostElevatorRegisterHandleForm.ids){
		var ids = lostElevatorRegisterHandleForm.ids;
		if(ids.length == null){
			if(ids.checked==true)
				return 0;
			else
...
function getVal(name,index){
	var obj = eval("lostElevatorRegisterHandleForm."+name);
	if(obj && obj.length){
		obj = obj[index];
	}
