...
function CreateToolBar(){
  ...
  <%-- 是否有可写的权限--%>
  <logic:equal name="<%=SysConfig.TOOLBAR_RIGHT%>" property="nlostelevatorregisterministerhandle" value="Y"> 
...
function returnMethod(){
    window.location = '<html:rewrite page="/lostElevatorRegisterMinisterHandleAction.do"/>?method=toSearchRecord';
}
...
function saveMethod(){
	var handleResult=document.getElementsByName("handleResult");
	  if(handleResult[0].value!=""){
		   document.lostElevatorRegisterMinisterHandleForm.isreturn.value = "N";
...
function saveReturnMethod(){
	var handleResult=document.getElementsByName("handleResult");
if(handleResult[0].value!=""){
	  document.lostElevatorRegisterMinisterHandleForm.isreturn.value = "Y";
    document.lostElevatorRegisterMinisterHandleForm.submit();
   }else
   {
	   var tile="<bean:message key='custreturnregistercheck.error.handleResult.required'/>";
