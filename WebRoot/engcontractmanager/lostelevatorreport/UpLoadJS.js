	 ...
	 // 处理返回信息函数
	    ...
	    function deleteFile(value,name,tb){
	 	    ...
	 	    if(confirm("确定删除吗？"+name)){
	 			...
	 			var uri = '<html:rewrite page="/lostElevatorReportAction.do"/>?method=toDelFileRecord';
	 			uri +='&filesid='+ value;
		...
		function downloadFile(id){
			var uri = '<html:rewrite page="/lostElevatorReportAction.do"/>?method=toDownloadFileRecord';
 			uri +='&filesid='+ id;
			window.location = uri;
			//window.open(url);
		}
