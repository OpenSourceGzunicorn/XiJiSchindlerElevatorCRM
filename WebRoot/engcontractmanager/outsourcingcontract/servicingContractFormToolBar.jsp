...
function returnMethod(){
  window.location = '<html:rewrite page="/outsourcingContractSearchAction.do"/>?method=toSearchRecord';
}
...
function saveMethod(){
	if(checkInput()){
		...
		if(elevatorNos !=null && elevatorNos.length>0){
			document.outsourcingContractForm.isreturn.value = "N";
...
function saveReturnMethod(){
	if(checkInput()){
	    var elevatorNos = document.getElementsByName("elevatorNo");
		if(elevatorNos !=null && elevatorNos.length>0){
			document.outsourcingContractForm.isreturn.value = "Y";
  			document.outsourcingContractForm.submit();
		}else{
			alert("不能保存，请添加电梯信息。");
		}
...
function saveMethod2(){
	if(checkInput()){
		var taskSubFlag = document.outsourcingContractForm.taskSubFlag.value;
...
function addElevators(tableId){
		...
		if(returnarray!=null && returnarray.length>0){
			...
			toSetInputValue(returnarray,"outsourcingContractForm");
		}			
}
...
//判断页面的值
function checkInput(){
	if(checkColumnInput(outsourcingContractForm) && checkRowInput('dynamictable_0','titleRow_0') ){
		return true;
	}else{
		return false;
	}
