...
var fileGlide=0;
  ...
  function AddRow(tableid){
	...
	var inp=createNode("2","checkbox","nodes",fileGlide,"","");
	td.appendChild(inp); 
...
//	inp=createNode("2","file","file_","","","");
  ...
  function createNode(flag, type ,name,value ,clas,checked){
	if(flag=="0"){//text node
		...
		return document.createElement("<input type=\'"+type+"\' name=\'"+name+"\' value=\'"+value+"\' "+checked+">");
	}
  }
  //复选框全选
 ...
 //--------------------------添加附件 结束---------------------
    ...
    function createXMLHttpRequest() {
	 ...
	 // 处理返回信息函数
	    ...
	    //删除
	    var tbs;
	    function deleteFile(value,tb){
	   
	 	    tbs=tb;
	 	    if(confirm("确定删除吗")){
	 			tbs.parentElement.parentElement.style.display='';
	 			var uri = '<html:rewrite page="/xifujAction.do"/>?method=toDeleteFileRecord';
	 			uri +='&filesid='+ value;
	 			uri +='&folder=ServicingContractQuoteMaster.file.upload.folder';
	 			uri +='&table=ContractFileinfo';
		...
		function downloadFile(id){
			var uri = '<html:rewrite page="/xifujAction.do"/>?method=toDownLoadFiles';
 			uri +='&filesid='+ id;
 			uri +='&folder=ServicingContractQuoteMaster.file.upload.folder';
 			uri +='&table=ContractFileinfo';
			window.location = uri;
