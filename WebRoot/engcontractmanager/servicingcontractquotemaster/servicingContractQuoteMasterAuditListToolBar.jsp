...
function CreateToolBar(){
  ...
  <%-- 是否有可写的权限--%>
  <logic:equal name="<%=SysConfig.TOOLBAR_RIGHT%>" property="nwgchangemasteraudit" value="Y">
...
if(serveTableForm.ids)
{
  ...
  if(l)
  {
    for(i=0;i<l;i++)
    {
      ...
      {
        window.location = '<html:rewrite page="/wgchangeAuditAction.do"/>?id='+document.serveTableForm.ids[i].value +'&method=toDisplayRecord';
        return;
      }
    }
    if(l >0)
  ...
  {
    window.location = '<html:rewrite page="/wgchangeAuditAction.do"/>?id='+document.serveTableForm.ids.value +'&method=toDisplayRecord';
  }
  else
  {
    alert("<bean:message key="javascript.role.alert2"/>");
	...
	if(serveTableForm.ids){
	 	...
	 	if(l)
	 	{
	 		for(i=0;i<l;i++)
	 		{
	 			if(document.serveTableForm.ids[i].checked == true){
	 				...
	 				if(hiddenstatus=="1"||hiddenstatus=="2"){
	 					...
	 					{
	 						window.location = '<html:rewrite page="/wgchangeAuditAction.do"/>?id='+document.serveTableForm.ids[i].value+'&method=toReStartProcess';
	 					}
	 					return;
	 				}else{
	 		...
	 		if(hiddenstatus=="1"||hiddenstatus=="2"){
	 			...
	 			{
	 				window.location = '<html:rewrite page="/wgchangeAuditAction.do"/>?id='+document.serveTableForm.ids.value+'&method=toReStartProcess';
	 			}
	 			return;
	 		}else{
	 			alert("请选择审核状态为'审核通过'或‘审批终止’的记录进行重启流程!");
