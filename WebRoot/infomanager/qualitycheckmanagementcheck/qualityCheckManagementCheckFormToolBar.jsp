...
function returnMethod(){
  window.location = '<html:rewrite page="/qualityCheckManagementCheckSearchAction.do"/>?method=toSearchRecord';
}
...
function downloadFile(name,oledName){
	var uri = '<html:rewrite page="/qualityCheckManagementCheckSearchAction.do"/>?method=toDownloadFileRecord';
...
function changeuser2(){
	...
	if(obj.value==""){
		...
		<logic:present name="userstorageidlist2">
			<logic:iterate id="element" name="userstorageidlist2">
			    var uid='<bean:write name="element" property="userid"/>';
			    var phone='<bean:write name="element" property="phone"/>';
			    if(maintper==uid){
			    	pphone.value=phone;
