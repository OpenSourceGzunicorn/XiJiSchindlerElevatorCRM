	 ...
	 // 处理返回信息函数
	    ...
	    function deleteFile(value,tb){
	 	    ...
	 	    if(confirm("确定删除吗")){
	 			...
	 			var uri = '<html:rewrite page="/PullDownAction.do"/>?method=toDelFileRecord';
	 			uri +='&filesid='+ value;
	 			uri +='&folder=engineQuoteProcess.file.upload.folder';
		...
		function downloadFile(id){
			var uri = '<html:rewrite page="/PullDownAction.do"/>?method=toDownloadFileRecord';
 			uri +='&filesid='+ id;
 			uri +='&folder=engineQuoteProcess.file.upload.folder';
			window.location = uri;
			//window.open(url);
