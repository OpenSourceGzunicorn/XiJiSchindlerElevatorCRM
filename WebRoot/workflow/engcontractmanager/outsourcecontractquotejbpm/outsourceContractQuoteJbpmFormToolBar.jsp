...
function saveMethod(){
	...
	if('${approve}' == "Y" || checkColumnInput(outsourceContractQuoteJbpmForm)){
		document.getElementById("SaveBtn").disabled=true;
		document.outsourceContractQuoteJbpmForm.submit();
	}
}
...
function addElevators(tableId){
	...
	if(returnarray!=null && returnarray.length>0){					
		...
		toSetInputValue(returnarray,"outsourceContractQuoteJbpmForm");
	}			
}
...
function checkInput(){  
	...
	if(elevatorNos !=null && elevatorNos.length>0){
		boo = checkColumnInput(outsourceContractQuoteJbpmForm) && checkRowInput("dynamictable_0","titleRow_0");
	}else{
		alert("不能保存，请添加合同明细。");
	}
	return boo;
