...
public class TechnologySupportMaintMinisterAction extends DispatchAction {

	Log log = LogFactory.getLog(TechnologySupportMaintMinisterAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		SysRightsUtil.filterModuleRight(request, response,
				SysRightsUtil.NODE_ID_FORWARD + "technologysupportmaintminister", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","申请技术支持(维保经理)处理 >> 查询列表");		
		...
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session
			...
			HTMLTableCache cache = new HTMLTableCache(session, "technologySupportMaintMinisterList");

			DefaultHTMLTable table = new DefaultHTMLTable();
			table.setMapping("fTechnologySupportMaintMinister");
			...
			String assignUserTel = tableForm.getProperty("assignUserTel");
			...
			if (!userInfo.getComID().equals("00")) {
				...
				if (userInfo.getStorageId() != null && !userInfo.getStorageId().equals("10000") ) {
			...
			try {
				...
				String sql="select "+StringUtils.join(colNames, ",")+"  from TechnologySupport ts left join LoginUser mslu on mslu.UserID=ts.MSProcessPeople left join LoginUser mmlu on mmlu.UserID=ts.MMProcessPeople left join LoginUser tslu on tslu.UserID=ts.TSProcessPeople,HotlineMotherboardType hmt,LoginUser lu,Company c,Storageid si where c.ComID=ts.MaintDivision and si.StorageID=ts.MaintStation and hmt.hmtId=ts.hmtId and ts.AssignUser=lu.UserID and ts.proStatus = '2' and ( ts.mmprocessPeople is null or ts.mmprocessPeople ='')";
			    ...
			    if (maintDivision != null && !maintDivision.equals("")) {
					String hql="select a from Storageid a,Company b where a.comid = b.comid and a.comid="+maintDivision+" and a.storagetype=1";
				...
				session.setAttribute("technologySupportMaintMinisterList", table);	
			...
			forward = mapping.findForward("technologySupportMaintMinisterList");
	...
	public ActionForward toDisplayRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","申请技术支持(维保经理)处理 >> 查看");
		...
		if (id != null) {
			try {
				...
				if (list != null && list.size() > 0) {
					...
					if(i.equals("Y"))
					{
						technologySupport.setMmisResolve("解决");
					}else{
					...
					if(i1.equals("Y"))
					{
						technologySupport.setMsisResolve("解决");
					}else{
						technologySupport.setMsisResolve("未解决");
					}
				    }
					String hql="from TechnologySupportFiles tsf where tsf.technologySupport.billno = "+technologySupport.getBillno();
			...
			forward = mapping.findForward("technologySupportMaintMinisterDisplay");
	...
	public ActionForward toPrepareUpdateRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","申请技术支持(维保经理)处理 >> 处理");
		DynaActionForm dform = (DynaActionForm) form;
		ActionErrors errors = new ActionErrors();
		...
		if (id != null) {
			try {
				hs = HibernateUtil.getSession();
				Query query = hs
						.createQuery("from TechnologySupport ts where ts.billno = :billno");
				query.setString("billno", id);
				List list = query.list();
                if(list!=null&&list.size()>0)
                {
                	...
                	technologySupportMaintStation.setMmprocessPeople(userInfo.getUserName());
                	technologySupportMaintStation.setMmprocessDate(DateUtil.getNowTime("yyyy-MM-dd HH:mm:ss"));
			...
			forward = mapping.findForward("technologySupportMaintMinisterModify");
     }
	...
	public ActionForward toUpdateRecord(ActionMapping mapping, ActionForm form,
		...
		String mmisResolve = (String) dform.get("mmisResolve");
		String rem = (String) dform.get("mmprocessRem");
		String mmDate=(String) dform.get("mmprocessDate");
		TechnologySupport technologySupportMaintStation=null;
		try {
			hs = HibernateUtil.getSession();
			...
			if (mmisResolve!= null&& !mmisResolve.trim().equals("")) {
				technologySupportMaintStation.setProStatus("4");
				if(mmisResolve.trim().equals("N")){
					technologySupportMaintStation.setProStatus("3");
				}
				technologySupportMaintStation.setMmisResolve(mmisResolve);
				technologySupportMaintStation.setMmprocessRem(rem);
				technologySupportMaintStation.setMmprocessDate(mmDate);
				technologySupportMaintStation.setMmprocessPeople(userInfo.getUserID());
