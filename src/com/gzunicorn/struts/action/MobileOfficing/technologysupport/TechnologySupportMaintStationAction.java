...
public class TechnologySupportMaintStationAction extends DispatchAction {

	Log log = LogFactory.getLog(TechnologySupportMaintStationAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil
				.filterModuleRight(request, response,
						SysRightsUtil.NODE_ID_FORWARD
								+ "technologysupportmaintstation", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", "申请技术支持(维保站长)处理 >> 查询列表");
		...
		HTMLTableCache cache = new HTMLTableCache(session,
				"technologySupportMaintStationList");

		DefaultHTMLTable table = new DefaultHTMLTable();
		table.setMapping("fTechnologySupportMaintStation");
		...
		try {
			...
			String sql="select "+StringUtils.join(colNames, ",")+"  "
					...
					+ "and ts.proStatus = '1' and (ts.msprocessPeople is null or ts.msprocessPeople ='')";
			...
			if (maintDivision != null && !maintDivision.equals("")) {
				String hql = "select a from Storageid a,Company b where a.comid = b.comid and a.comid="
						+ maintDivision + " and a.storagetype=1";
			...
			session.setAttribute("technologySupportMaintStationList", table);
		...
		forward = mapping.findForward("technologySupportMaintStationList");
	...
	public ActionForward toDisplayRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location", "申请技术支持(维保站长)处理 >> 查看");
		...
		if (id != null) {
			try {
				...
				if (list != null && list.size() > 0) {
					...
					String hql="from TechnologySupportFiles tsf where tsf.billno = "+technologySupport.getBillno();
			...
			forward = mapping.findForward("technologySupportMaintStationDisplay");
	...
	public ActionForward toPrepareUpdateRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","申请技术支持(维保站长)处理 >> 处理");
		DynaActionForm dform = (DynaActionForm) form;
		ActionErrors errors = new ActionErrors();
		...
		if (id != null) {
			try {
                ...
                if(list!=null&&list.size()>0)
                {
                	...
                	technologySupportMaintStation.setMsprocessPeople(userInfo.getUserName());
                	technologySupportMaintStation.setMsprocessDate(DateUtil.getNowTime("yyyy-MM-dd HH:mm:ss"));
			...
			forward = mapping.findForward("technologySupportMaintStationModify");
     }
	...
	public ActionForward toUpdateRecord(ActionMapping mapping, ActionForm form,
		...
		String msisResolve = (String) dform.get("msisResolve");
		String rem = (String) dform.get("msprocessRem");
		String msDate=(String) dform.get("msprocessDate");
		TechnologySupport technologySupportMaintStation=null;
		try {
			hs = HibernateUtil.getSession();
			...
			if (msisResolve!= null&& !msisResolve.trim().equals("")) {
				
				technologySupportMaintStation.setProStatus("4");
				if(msisResolve.trim().equals("N")){
					technologySupportMaintStation.setProStatus("2");
				}
				technologySupportMaintStation.setMsisResolve(msisResolve);
				technologySupportMaintStation.setMsprocessRem(rem);
				technologySupportMaintStation.setMsprocessDate(msDate);
				technologySupportMaintStation.setMsprocessPeople(userInfo.getUserID());
	...
	public void toStorageIDList(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (comid != null && !"".equals(comid)) {
				...
				String hql = "select a from Storageid a,Company b where a.comid = b.comid and a.comid="
						+ comid + " and a.storagetype=1";
				if (!userInfo.getComID().equals("00")) {
					if (userInfo.getStorageName() != null) {
						String mainStation = bd.getName(hs,"StorageID", "StorageID",
								"StorageName", userInfo.getStorageName().trim() + "%");
						hql += "and a.storageid=" + mainStation;
					}
				}
