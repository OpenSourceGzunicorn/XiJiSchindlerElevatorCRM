...
public class CustomerAction extends DispatchAction {
	...
	Log log = LogFactory.getLog(CustomerAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if(!"toDisplayRecord".equals(name)){
			/** **********开始用户权限过滤*********** */
			SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "customer", null);
			/** **********结束用户权限过滤*********** */
		}
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","客户信息管理 >> 查询列表");		
		...
		if (tableForm.getProperty("genReport") != null
			...
			HTMLTableCache cache = new HTMLTableCache(session, "customerListm");

			DefaultHTMLTable table = new DefaultHTMLTable();
			table.setMapping("fCustomer");
			...
			try {
				...
				String sql="from Customer where cusNature='JF' ";
				if (companyId != null && !companyId.trim().equals("")) {
					sql += " and companyId like '%"+companyId.trim()+"%'";
				}
				...
				if (enabledFlag != null && !enabledFlag.trim().equals("")) {
					sql += " and enabledFlag ='"+enabledFlag.trim()+"'";
				}
				if (maintContractNo != null && !maintContractNo.trim().equals("")) {
					sql += " and companyId in(select companyId from MaintContractMaster where maintContractNo like '%"+maintContractNo.trim()+"%')";
				}
				...
				session.setAttribute("customerListm", table);
			...
			forward = mapping.findForward("customerList");
		}
		return forward;
	}
	...
	public ActionForward toDisplayRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","客户信息管理 >> 查看");
		...
		if (id != null) {
			try {
				hs = HibernateUtil.getSession();
				customer = (Customer) hs.get(Customer.class, (String) dform.get("id"));

				if (customer == null) {
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("customer.display.recordnotfounterror"));
				}
			} catch (DataStoreException e) {
				e.printStackTrace();
			} catch (Exception e1) {
			...
			forward = mapping.findForward("customerDisplay");
	...
	public ActionForward toPrepareAddRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","客户信息管理 >> 添加");
		DynaActionForm dform = (DynaActionForm) form;
		...
		try{
		   ...
		   String sql="select MAX(c.companyId) from Customer c where C.CusNature='JF' ";
	...
	public ActionForward toAddRecord(ActionMapping mapping, ActionForm form,
		...
		String r1 = (String) dform.get("r1");//开票类型

		try {
			hs = HibernateUtil.getSession();
			tx = hs.beginTransaction();

			String sql="from Customer where companyName='"+companyName.trim()+"' and cusNature='JF'";
			...
			if(sqlist!=null && sqlist.size()>0){
				...
				customer.setRem(rem);
				customer.setCusNature("JF");
				customer.setInvoiceName(invoiceName);//开票名称
				customer.setR1(r1);//开票类型
	...
	public ActionForward toPrepareUpdateRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","客户信息管理 >> 修改");
		DynaActionForm dform = (DynaActionForm) form;
		ActionErrors errors = new ActionErrors();
		...
		//System.out.println(dform.get("id"));
		Session hs = null;
		Customer customer = null;
		if (id != null) {
			try {
			...
			request.setAttribute("customerBean", customer);
			forward = mapping.findForward("customerModify");
		}
	...
	public ActionForward toUpdateRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String sql="from Customer where companyName='"+companyName.trim()+"' and cusNature='JF' and companyId not in('"+companyId+"')";
			List sqlist=hs.createQuery(sql).list();
			if(sqlist!=null && sqlist.size()>0){
				errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.string","<font color='red'>单位名称 已经存在，保存失败！</font>"));
			}else{
	...
	public HttpServletResponse toExcelRecord(ActionForm form,
		...
		try {
			...
			Criteria criteria = hs.createCriteria(Customer.class).add(Expression.eq("cusNature", "JF"));
			...
			if (roleList != null && !roleList.isEmpty()) {
				...
				//cell0.setEncoding(HSSFCell.ENCODING_UTF_16);
				cell0.setCellValue("纳税人识别号");
				
				cell0 = row0.createCell((short)17);
				//cell0.setEncoding(HSSFCell.ENCODING_UTF_16);
				cell0.setCellValue("开票类型");
				...
				for (int i = 0; i < l; i++) {
					...
					String r1=customer.getR1();
					String r1name="";
					if(r1!=null && r1.trim().equals("Z")){
						r1name="专票";
					}else if(r1!=null && r1.trim().equals("P")){
						r1name="普票";
					}
					cell = row.createCell((short)17);
					//cell.setEncoding(HSSFCell.ENCODING_UTF_16);
					cell.setCellValue(r1name);
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("客户信息管理", "utf-8") + ".xlsx");
	 ...
	 * @return
	 */
	public ActionForward toDisplay(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		
		request.setAttribute("navigator.location","西继迅达信息 >> 查看");
		DynaActionForm dform = (DynaActionForm) form;
		ActionErrors errors = new ActionErrors();
		ActionForward forward = null;
		Session hs = null;
		...
		try {
				...
				hs = HibernateUtil.getSession();
				String sql="select c.companyID from Customer c where c.cusNature='XJS'";
				List list=hs.createSQLQuery(sql).list();
				customer = (Customer) hs.get(Customer.class, (Serializable) list.get(0));
			...
			request.setAttribute("display", "yes");
			request.setAttribute("customerBean", customer);
			forward = mapping.findForward("xjscrmMessageDisplay");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ActionForward toPrepareUpdate(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		
		request.setAttribute("navigator.location","西继迅达信息 >> 编辑");
		DynaActionForm dform = (DynaActionForm) form;
		ActionErrors errors = new ActionErrors();
		...
		if (id != null) {
			...
			request.setAttribute("customerBean", customer);
			forward = mapping.findForward("xjscrmMessageModify");
		}
	...
	public ActionForward toUpdate(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (dform.get("id") != null
					&& dform.get("companyId") != null
					&& !((String) dform.get("id")).equals((String) dform.get("companyId"))) {
				hs.delete(customer);
			...
			customer.setEnabledFlag(enabledFlag);
			customer.setRem(rem);
		//	customer.setCusNature("XJS");
			customer.setInvoiceName(invoiceName);//开票名称
			customer.setOperId(userInfo.getUserID());//录入人
			customer.setOperDate(CommonUtil.getToday());//录入时间
			request.setAttribute("id", (String) dform.get("companyId"));
		...
		try {
			if (isreturn != null && isreturn.equals("Y") && errors.isEmpty()) {
				...
				forward = mapping.findForward("returnxjscrmDisplay");
			} else {
				// return modify page
				if (errors.isEmpty()) {
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("update.success"));
				...
				forward = mapping.findForward("returnxjscrmModify");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ActionForward toPrepareImportRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","客户信息管理 >> 导入");
		DynaActionForm dform = (DynaActionForm) form;
		if (request.getAttribute("error") == null
				|| request.getAttribute("error").equals("")) {
			dform.initialize(mapping);
		}
		return mapping.findForward("customerImport");
	}
		...
		try {
			...
			if (fileFromt!=null && fileFromt.equals("xlsx")) {//excel 2007
				...
				for(int rowNum = 1; rowNum < rowSum; rowNum++){					
				    ...
				    String accountHolder=cellValueToString(row, 12);//地址、电话
				    String account=cellValueToString(row, 13);//银行帐号
				    String taxId=cellValueToString(row, 14);//纳税人识别号
				    String r1=cellValueToString(row, 15);//开票类型
				    String rem=cellValueToString(row, 16);//备注
				    
				    if(r1!=null && r1.trim().equals("专票")){
				    	r1="Z";
				    }else if(r1!=null && r1.trim().equals("普票")){
				    	r1="P";
				    }
				...
				if(reStr == null || reStr.length() == 0){
					String hql = "select companyId,companyName from Customer where cusNature='JF' and companyName in ("+id1+")"; //删除旧版本数据
