...
public class gzImportAction extends DispatchAction {

	Log log = LogFactory.getLog(gzImportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "gzimport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","工资导入 >> 查询列表");		
		...
		HTMLTableCache cache = new HTMLTableCache(session, "gzImporModifyList");

		DefaultHTMLTable table = new DefaultHTMLTable();
		table.setMapping("fgzImportSearch");
		...
		try {
			...
			String sql = "select a.numno,c.comname,s.storagename,a.yearMonth,a.gzMoney,a.dayMoney,b.username,a.OperDate "
					+ "from gzImportInfo a,Loginuser b,Company c,storageid s "
			...
			session.setAttribute("gzImporModifyList", table);
		...
		forward = mapping.findForward("gzImportList");
	...
	public ActionForward toDeleteRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String delsql="delete from gzImportInfo where numno="+id;
	...
	public ActionForward toPrepareImportRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","工资导入 >> 导入");
		DynaActionForm dform = (DynaActionForm) form;
		if (request.getAttribute("error") == null
				|| request.getAttribute("error").equals("")) {
			dform.initialize(mapping);
		}
		return mapping.findForward("gzImportImport");
	}
	...
	public ActionForward toImportRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (fileFromt!=null && fileFromt.equals("xlsx")) {//excel 2007
				...
				if(reStr == null || reStr.length() == 0){
					String sqldel="delete gzImportInfo where MaintDivision=? and MaintStation=? and yearMonth=?";
					pstmtdel=hs.connection().prepareStatement(sqldel);
					String sqlins="insert into gzImportInfo(OperId,OperDate,MaintDivision,MaintStation,yearMonth,gzMoney,dayMoney) "
							+ "values(?,?,?,?,?,?,?)";
