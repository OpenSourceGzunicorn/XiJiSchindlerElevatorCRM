...
public class nzjImportAction extends DispatchAction {

	Log log = LogFactory.getLog(nzjImportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "nzjimport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","年终奖导入 >> 查询列表");		
		...
		HTMLTableCache cache = new HTMLTableCache(session, "nzjImporModifyList");

		DefaultHTMLTable table = new DefaultHTMLTable();
		table.setMapping("fnzjImportSearch");
		...
		try {
			...
			String sql = "select a.numno,c.comname,s.storagename,a.years,a.nzjMoney,a.dayMoney,b.username,a.OperDate "
					+ "from nzjImportInfo a,Loginuser b,Company c,storageid s "
			...
			session.setAttribute("nzjImporModifyList", table);
		...
		forward = mapping.findForward("nzjImportList");
	...
	public ActionForward toDeleteRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String delsql="delete from nzjImportInfo where numno="+id;
	...
	public ActionForward toPrepareImportRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","年终奖导入 >> 导入");
		DynaActionForm dform = (DynaActionForm) form;
		if (request.getAttribute("error") == null
				|| request.getAttribute("error").equals("")) {
			dform.initialize(mapping);
		}
		return mapping.findForward("nzjImportImport");
	}
	...
	public ActionForward toImportRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (fileFromt!=null && fileFromt.equals("xlsx")) {//excel 2007
				...
				if(reStr == null || reStr.length() == 0){
					String sqldel="delete nzjImportInfo where MaintDivision=? and MaintStation=? and years=?";
					pstmtdel=hs.connection().prepareStatement(sqldel);
					String sqlins="insert into nzjImportInfo(OperId,OperDate,MaintDivision,MaintStation,years,nzjMoney,dayMoney) "
							+ "values(?,?,?,?,?,?,?)";
