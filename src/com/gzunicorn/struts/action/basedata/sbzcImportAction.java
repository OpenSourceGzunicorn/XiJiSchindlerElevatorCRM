...
public class sbzcImportAction extends DispatchAction {

	Log log = LogFactory.getLog(sbzcImportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "sbzcimport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","社保支出导入 >> 查询列表");		
		...
		HTMLTableCache cache = new HTMLTableCache(session, "sbzcImporModifyList");

		DefaultHTMLTable table = new DefaultHTMLTable();
		table.setMapping("fsbzcImportSearch");
		...
		try {
			...
			String sql = "select a.numno,c.comname,s.storagename,a.yearMonth,a.sbzcMoney,a.dayMoney,b.username,a.OperDate "
					+ "from sbzcImportInfo a,Loginuser b,Company c,storageid s "
			...
			session.setAttribute("sbzcImporModifyList", table);
		...
		forward = mapping.findForward("sbzcImportList");
	...
	public ActionForward toDeleteRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String delsql="delete from sbzcImportInfo where numno="+id;
	...
	public ActionForward toPrepareImportRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","社保支出导入 >> 导入");
		DynaActionForm dform = (DynaActionForm) form;
		if (request.getAttribute("error") == null
				|| request.getAttribute("error").equals("")) {
			dform.initialize(mapping);
		}
		return mapping.findForward("sbzcImportImport");
	}
	...
	public ActionForward toImportRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (fileFromt!=null && fileFromt.equals("xlsx")) {//excel 2007
				...
				if(reStr == null || reStr.length() == 0){
					String sqldel="delete sbzcImportInfo where MaintDivision=? and MaintStation=? and yearMonth=?";
					pstmtdel=hs.connection().prepareStatement(sqldel);
					String sqlins="insert into sbzcImportInfo(OperId,OperDate,MaintDivision,MaintStation,yearMonth,sbzcMoney,dayMoney) "
							+ "values(?,?,?,?,?,?,?)";
