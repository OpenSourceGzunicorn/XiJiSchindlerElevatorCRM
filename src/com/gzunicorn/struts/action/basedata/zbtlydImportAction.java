...
public class zbtlydImportAction extends DispatchAction {

	Log log = LogFactory.getLog(zbtlydImportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "zbtlydimport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","在保台量月度导入 >> 查询列表");		
		...
		HTMLTableCache cache = new HTMLTableCache(session, "zbtlydImporModifyList");

		DefaultHTMLTable table = new DefaultHTMLTable();
		table.setMapping("fzbtlydImportSearch");
		...
		try {
			...
			String sql = "select a.numno,c.comname,s.storagename,a.yearMonth,a.totalnum,a.daynum,b.username,a.OperDate "
					+ "from zbtlydImportInfo a,Loginuser b,Company c,storageid s "
			...
			if (yearMonth != null && !yearMonth.equals("")) {
				sql += " and a.yearMonth like '"+yearMonth.trim()+"'";
			}
			...
			for (Object object : list) {
				...
				master.put("yearMonth",objs[3]);
				master.put("totalnum",objs[4]);
				master.put("daynum",objs[5]);
			...
			session.setAttribute("zbtlydImporModifyList", table);
		...
		forward = mapping.findForward("zbtlydImportList");
	...
	public ActionForward toDeleteRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String delsql="delete from zbtlydImportInfo where numno="+id;
	...
	public ActionForward toPrepareImportRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","在保台量月度导入 >> 导入");
		DynaActionForm dform = (DynaActionForm) form;
		if (request.getAttribute("error") == null
				|| request.getAttribute("error").equals("")) {
			dform.initialize(mapping);
		}
		return mapping.findForward("zbtlydImportImport");
	}
	...
	public ActionForward toImportRecord(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (fileFromt!=null && fileFromt.equals("xlsx")) {//excel 2007
				...
				if(reStr == null || reStr.length() == 0){
					String sqldel="delete zbtlydImportInfo where MaintDivision=? and MaintStation=? and yearMonth=?";
					pstmtdel=hs.connection().prepareStatement(sqldel);
					String sqlins="insert into zbtlydImportInfo(OperId,OperDate,MaintDivision,MaintStation,yearMonth,totalnum,daynum) "
							+ "values(?,?,?,?,?,?,?)";
