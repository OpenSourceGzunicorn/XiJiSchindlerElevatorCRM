 ...
 *
 */
public class ContractInvoiceManageAudit2Action extends DispatchAction {

	Log log = LogFactory.getLog(ContractInvoiceManageAudit2Action.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD +"contractinvoiceaudit2", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecordAudit(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","开票总部长审核>> 查询列表");		
		...
		if (tableForm.getProperty("genReport") != null
			...
			HTMLTableCache cache = new HTMLTableCache(session, "contractInvoiceAuditList2");

			DefaultHTMLTable table = new DefaultHTMLTable();
			table.setMapping("fContractInvoiceManageAudit2");
			...
			try {
				...
				String sql = "select jnlNo,ARF_JnlNo,invoiceNo,invoiceDate,invoiceType,invoiceMoney," +
						"rem,istbp,submitType,isnull(auditStatus2,'N'),maintDivision,contractNo,contractType " +
				...
				if (auditStatus != null && !auditStatus.equals("")) {
					sql += " and isnull(auditStatus2,'N') like '%"+auditStatus.trim()+"%'";
				}
				if (maintDivision != null && !maintDivision.equals("")) {
				...
				session.setAttribute("contractInvoiceAuditList2", table);
				//发票类型下拉框列表
			...
			forward = mapping.findForward("contractInvoiceManageAuditList2");
		}
		return forward;
	}
	...
	public ActionForward toAuditDisplay(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","开票总部长审核 >> 查看");
		ActionForward forward = null;
		...
		forward = mapping.findForward("contractInvoiceAuditDisplay2");
		return forward;
	}
	...
	public ActionForward toPrepareAuditRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","开票总部长审核 >> 审核");	
		...
		forward = mapping.findForward("contractInvoiceManageAudit2");
	...
	public ActionForward toAuditRecord(ActionMapping mapping, ActionForm form,
		...
		String auditRem=(String)dform.get("auditRem2");
		String auditStatus=(String)dform.get("auditStatus2");
		...
		try {
			...
			if(master!=null){
				...
				master.setAuditStatus2(auditStatus);
		...
		try {
			if (isreturn != null && isreturn.equals("Y") && errors.isEmpty()) {
				...
				forward = mapping.findForward("returnAuditList2");
			} else {
				// return modify page
				if (errors.isEmpty()) {
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("update.success"));
