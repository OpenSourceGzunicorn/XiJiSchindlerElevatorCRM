 ...
 *
 */
public class ContractInvoiceManageAudit4Action extends DispatchAction {

	Log log = LogFactory.getLog(ContractInvoiceManageAudit4Action.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD +"contractinvoiceaudit4", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecordAudit(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","开票财务总监审核>> 查询列表");		
		...
		if (tableForm.getProperty("genReport") != null
			...
			HTMLTableCache cache = new HTMLTableCache(session, "contractInvoiceAuditList4");

			DefaultHTMLTable table = new DefaultHTMLTable();
			table.setMapping("fContractInvoiceManageAudit4");
			...
			try {
				...
				String sql = "select jnlNo,ARF_JnlNo,invoiceNo,invoiceDate,invoiceType,invoiceMoney," +
						"rem,istbp,submitType,isnull(auditStatus4,'N'),maintDivision,contractNo,contractType " +
				...
				if (auditStatus != null && !auditStatus.equals("")) {
					sql += " and isnull(auditStatus4,'N') like '"+auditStatus.trim()+"'";
				}
				if (maintDivision != null && !maintDivision.equals("")) {
				...
				session.setAttribute("contractInvoiceAuditList4", table);
				//发票类型下拉框列表
			...
			forward = mapping.findForward("contractInvoiceManageAuditList4");
		}
		return forward;
	}
	...
	public ActionForward toAuditDisplay(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","开票财务总监审核 >> 查看");
		ActionForward forward = null;
		...
		forward = mapping.findForward("contractInvoiceAuditDisplay4");
		return forward;
	}
	...
	public ActionForward toPrepareAuditRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","开票财务总监审核 >> 审核");	
		...
		forward = mapping.findForward("contractInvoiceManageAudit4");
	...
	public ActionForward toAuditRecord(ActionMapping mapping, ActionForm form,
		...
		String auditRem=(String)dform.get("auditRem4");
		String auditStatus=(String)dform.get("auditStatus4");
		...
		try {
			...
			if(master!=null){
				...
				master.setAuditStatus4(auditStatus);
				if(submitType!=null && submitType.trim().equals("R")){
					...
					master.setAuditOperid4(userInfo.getUserID());
					master.setAuditDate4(CommonUtil.getNowTime());
					master.setAuditRem4(auditRem);
				}
				hs.save(master);
			}
			tx.commit();
		...
		try {
			if (isreturn != null && isreturn.equals("Y") && errors.isEmpty()) {
				...
				forward = mapping.findForward("returnAuditList4");
			} else {
				// return modify page
				if (errors.isEmpty()) {
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("update.success"));
