...
public class InvoiceAuditAction extends DispatchAction {

	Log log = LogFactory.getLog(InvoiceAuditAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		SysRightsUtil.filterModuleRight(request, response,
				SysRightsUtil.NODE_ID_FORWARD + "invoiceaudit", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","收票审核 >> 查询列表");		
		...
		if (tableForm.getProperty("genReport") != null
			...
			HTMLTableCache cache = new HTMLTableCache(session, "invoiceAuditList");

			DefaultHTMLTable table = new DefaultHTMLTable();
			table.setMapping("fInvoiceAudit");
			...
			try {
				...
				String sql = "select ctm,c.companyName,compay.comfullname,it.inTypeName from ContractTicketManage ctm,Customer c,Company compay,InvoiceType it where it.inTypeId=ctm.invoiceType and compay.comid=ctm.maintDivision and ctm.companyId=c.companyId and ctm.submitType='Y'";
				...
				List invoiceAuditList = new ArrayList();
				for (Object object : list) {
					...
					invoiceAuditList.add(ctm);
				}

				table.addAll(invoiceAuditList);
				session.setAttribute("invoiceAuditList", table);
				// 分部下拉框列表
			...
			forward = mapping.findForward("invoiceAuditList");
		}
		return forward;
	}
	...
	public ActionForward toPrepareUpdateRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","收票审核 >> 查看与审核");	
		...
		forward = mapping.findForward("invoiceAuditModify");
	...
	public ActionForward toUpdateRecord(ActionMapping mapping, ActionForm form,
		...
		if (id != null && !id.equals("")) {
			...
			try {
				...
				ContractTicketManage master = (ContractTicketManage) hs
						.get(ContractTicketManage.class, id);
	...
	public ActionForward toDisplayRecord(ActionMapping mapping,
		...
		forward = mapping.findForward("invoiceAuditDisplay");
		return forward;
	}
		...
		if (id != null && !id.equals("")) {
			try {
						...
						if (list != null && list.size() > 0) {					
							...
							request.setAttribute("invoiceAuditBean", master);	
