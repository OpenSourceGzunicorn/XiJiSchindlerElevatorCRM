 ...
 *
 */
public class LostElevatorReportAudit2Action extends DispatchAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "lostelevatorreportaudit2", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location","维保合同退保分部长审核 >> 查询列表");		
		...
		HTMLTableCache cache = new HTMLTableCache(session, "lostElevatorReportAudit2List");

		DefaultHTMLTable table = new DefaultHTMLTable();
		table.setMapping("fLostElevatorReportAudit2");
...
//		String submitType = tableForm.getProperty("submitType");// 提交标志
		...
		try {
			...
			String sql = "select a,b.comname,c.pullname,s.storagename " +
					...
					" and isnull(a.fhRem,'')<>'' "+//提交的并且有回访结果的
			...
			if (auditStatus != null && !auditStatus.equals("")) {
				sql += " and isnull(a.auditStatus2,'N') like '"+auditStatus.trim()+"'";
			}
			...
			session.setAttribute("lostElevatorReportAudit2List", table);
			// 分部下拉框列表
		...
		forward = mapping.findForward("lostElevatorReportAudit2List");
	...
	public ActionForward toDisplayRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","维保合同退保分部长审核 >> 查看");
		...
		if (id != null) {
			try {
				...
				if (list != null && list.size() > 0) {
					...
					if(report.getAuditStatus2()==null || report.getAuditStatus2().equals("N")){
						report.setAuditDate2(CommonUtil.getNowTime());
						report.setAuditOperid2(userInfo.getUserName());
						report.setAuditStatus2("N");
					}else{
			...
			forward = mapping.findForward("lostElevatorReportAudit2Display");
		}		
	...
	public ActionForward toAuditRecord(ActionMapping mapping, ActionForm form,
		...
		if(isTokenValid(request, true)){
			...
			DynaActionForm dform = (DynaActionForm) form;				
			String id = request.getParameter("id"); 		
			String auditStatus = String.valueOf(dform.get("auditStatus2")); // 审核状态
			String submitType = "N".equals(auditStatus) ? "R" : "Y"; // 是否驳回
			String auditRem = String.valueOf(dform.get("auditRem2")); // 审核意见
			String isCharge = String.valueOf(dform.get("isCharge")); // 是否扣款
			...
			if(id != null && !id.equals("")){
				...
				try {
					...
					report.setSubmitType(submitType);//驳回给创建人
					report.setAuditDate2(CommonUtil.getNowTime());
					report.setAuditOperid2(userInfo.getUserID());
					report.setAuditRem2(auditRem);
					report.setAuditStatus2(submitType);
					
					if("Y".equals(submitType)){
						report.setIsCharge(isCharge);
						report.setAuditStatus3(null);
						report.setAuditDate3("");
						report.setAuditOperid3("");
						//report.setAuditRem3("");
					}
