...
public class hotCalloutAudit  extends DispatchAction{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if(!"toDisplayRecord".equals(name)){
			/** **********开始用户权限过滤*********** */
			SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "hotcalloutaudit", null);
			/** **********结束用户权限过滤*********** */
		}
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		request.setAttribute("navigator.location","急修安全经理审核 >> 查询列表");		
		...
		if (tableForm.getProperty("genReport") != null
			...
			HTMLTableCache cache = new HTMLTableCache(session, "hotCalloutAuditList");

			DefaultHTMLTable table = new DefaultHTMLTable();
			table.setMapping("fHotCalloutAudit");
			...
			try {
			   ...
			   cache.check(table);
			    list3.addAll(list2);				
				table.addAll(list3);
				session.setAttribute("hotCalloutAuditList", table);
	...
	public ActionForward toDisplayRecord(ActionMapping mapping,
		...
		request.setAttribute("navigator.location","急修安全经理审核 >> 审核");
	   ...
	   if (id != null) {				
		try {
			...
			if(typejsp!=null && "sh".equals(typejsp)){
				cm.setSmAuditOperid(userInfo.getUserName());
				cm.setSmAuditDate(xj.getdatetime());
	...
	if (!errors.isEmpty()) {
		...
		forward = mapping.findForward("toAudit");
		return forward;
	...
	public ActionForward tosaveshps(ActionMapping mapping, ActionForm form,
		...
		if(calloutMasterNo!=null){
			try {
				...
				String smAuditRem=request.getParameter("smAuditRem");
				...
				cm.setIsSendSms(isSendSms);
				cm.setHfcId(hfcId);
				cm.setHandleStatus("6");
				
				cm.setSmAuditOperid(userInfo.getUserID());
				cm.setSmAuditDate(xj.getdatetime());
				cm.setSmAuditRem(smAuditRem);
				
				hs.save(cp);
				hs.save(cm);
			    hs.flush();
				if(isSendSms!=null && "Y".equals(isSendSms)){
					/******************发送短信开始**************************************/					
