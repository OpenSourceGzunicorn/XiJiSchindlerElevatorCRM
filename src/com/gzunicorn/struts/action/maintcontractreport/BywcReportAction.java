 ...
 *
 */
public class BywcReportAction extends DispatchAction {

	Log log = LogFactory.getLog(BywcReportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "bywcreport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 保养完成报表 >> 查询");
		...
		forward = mapping.findForward("bywcReportSearch");		
		return forward;
	}
	...
	public ActionForward toSearchResults(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 保养完成报表 >> 查询结果");
		ActionForward forward = null;
		HttpSession session = request.getSession();
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session.getAttribute(SysConfig.LOGIN_USER_INFO);
		DynaActionForm dform = (DynaActionForm) form;
		...
		try {
			...
			String hql="exec sp_bywcReport '"+maintdivision.trim()+"','"+maintstation.trim()+"','"
					+maintPersonnel.trim()+"','"+sdate1.trim()+"','"+edate1.trim()+"'";
			System.out.println("保养完成报表>>>>"+hql);
            ...
            while(rs.next()){
            	...
            	map.put("ComName", rs.getString("ComName"));//维保分部
            	map.put("StorageName", rs.getString("StorageName"));//维保站
            	map.put("idCardNo", rs.getString("idCardNo"));//身份证
            	map.put("maintpersonnel", rs.getString("maintpersonnel"));
            	//map.put("idCardNo", bd.getName("Loginuser", "idcardno", "userid", rs.getString("maintpersonnel")));//身份证
            	//map.put("maintpersonnel", bd.getName("Loginuser", "username", "userid", rs.getString("maintpersonnel")));
            	map.put("planmaintnum", rs.getString("planmaintnum"));//计划保养次数
            	map.put("afinishtimes", rs.getString("afinishtimes"));//实际完成次数
            	map.put("zeroscoretimes", rs.getString("zeroscoretimes"));//得分为0次数
            	map.put("averagescore", rs.getString("averagescore"));//平均保养得分
            	map.put("shijibili", rs.getString("shijibili"));//实际完成比例
            	map.put("elenum", rs.getString("elenum"));//变红台数
            ...
            hmap.put("sdate1", sdate1);
        	hmap.put("edate1", edate1);
            ...
            if (dform.get("genReport") != null && dform.get("genReport").equals("Y")) {
                ...
                if(reportList.size()>0 && reportList.size()<3001){
                	request.setAttribute("maintenanceBywcReportList", reportList);
    			...
    			forward = mapping.findForward("bywcReportList");	
    		}
		}catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public HttpServletResponse toExcelRecord(HttpServletResponse response,
		...
		String[] titlename={"维保分部","维保站","维保工身份证号","维保工","计划保养次数","实际完成次数","得分为0次数","实际完成比例(%)","平均保养得分","变红台数"};
		String[] titleid={"ComName","StorageName","idCardNo","maintpersonnel","planmaintnum","afinishtimes","zeroscoretimes","shijibili","averagescore","elenum"};
		
		XSSFWorkbook wb = new XSSFWorkbook();
        XSSFSheet sheet = wb.createSheet("保养完成报表");
        ...
        cs.setFont(f);
        String sdate1 = (String)hmap.get("sdate1");
        String edate1 = (String)hmap.get("edate1");
        int toolnum=2;
        XSSFRow row1 = sheet.createRow( 0);
        XSSFCell cell1 = row1.createCell((short)0);
        cell1.setCellValue("维保计划日期："+sdate1+"至"+edate1);
        //创建标题
		...
		if (ReportList != null && ReportList.size()>0) {
			...
			for (int j = 0; j < ReportList.size(); j++) {
				...
				for(int c=0;c<titleid.length;c++){
				    ...
				    if(titleid[c].equals("planmaintnum") || titleid[c].equals("afinishtimes") 
				    		|| titleid[c].equals("zeroscoretimes") || titleid[c].equals("averagescore")
				    		|| titleid[c].equals("elenum")){
				    	cell.setCellValue(Double.parseDouble((String)map.get(titleid[c])));//有空值
				    }else{
...
/*		sheet.addMergedRegion(new CellRangeAddress(toolnum, toolnum, 0, 8));
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("保养完成报表", "utf-8") + ".xlsx");
