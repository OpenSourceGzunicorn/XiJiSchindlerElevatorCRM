 ...
 *
 */
public class calloutMasterIsStopReportAction extends DispatchAction {

	Log log = LogFactory.getLog(calloutMasterIsStopReportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "calloutmasterisstopreport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 停梯报表 >> 查询");
		...
		forward = mapping.findForward("calloutMasterIsStopSearch");		
		return forward;
	}
	...
	public ActionForward toSearchResults(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 停梯报表 >> 查询结果");
		ActionForward forward = null;
		HttpSession session = request.getSession();
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session.getAttribute(SysConfig.LOGIN_USER_INFO);
		DynaActionForm dform = (DynaActionForm) form;
		...
		try {
			...
			String hql="exec sp_IsStopReport '"+maintdivision.trim()+"','"+maintstation.trim()+"','"+sdate1.trim()+"','"+edate1.trim()+"'";
			System.out.println("停梯报表>>>>"+hql);
            ...
            while(rs.next()){
            	...
            	map.put("stoprem", rs.getString("stoprem"));
            ...
            if (dform.get("genReport") != null && dform.get("genReport").equals("Y")) {
                ...
                if(reportList.size()>0 && reportList.size()<3001){
                	request.setAttribute("isStopReportList", reportList);
    			...
    			forward = mapping.findForward("calloutMasterIsStopList");	
    		}
		}catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public HttpServletResponse toExcelRecord(HttpServletResponse response,
		...
		String[] titlename={"急修编号","维保分部门","维保站","报修时间","处理人","报修单位名称","电梯编号","停梯备注"};
		String[] titleid={"calloutmasterno","comname","storagename","repairtime","username","companyname","elevatorno","stoprem"};
		
		XSSFWorkbook wb = new XSSFWorkbook();
        XSSFSheet sheet = wb.createSheet("停梯报表");
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("停梯报表", "utf-8") + ".xlsx");
