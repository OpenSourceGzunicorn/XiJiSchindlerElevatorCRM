package com.gzunicorn.struts.action.report.grossmarginreport;
 ...
 *
 */
public class ContractGrossMarginReportAction extends DispatchAction {

	Log log = LogFactory.getLog(ContractGrossMarginReportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "contractgrossmarginreport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 维保合同毛利计算报表 >> 查询");
		...
		forward = mapping.findForward("contractGrossMarginReportSearch");		
		return forward;
	}
	...
	public ActionForward toSearchResults(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 维保合同毛利计算报表 >> 查询结果");
		ActionForward forward = null;
		HttpSession session = request.getSession();
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session.getAttribute(SysConfig.LOGIN_USER_INFO);
		DynaActionForm dform = (DynaActionForm) form;
		...
		String contractno = request.getParameter("contractno");//维保合同号
		...
		if(salescontractno==null || salescontractno.trim().equals("")){
			...
			salescontractno="%"+salescontractno.trim()+"%";
		}
		String sdate = request.getParameter("sdate");//毛利统计日期
		if(sdate==null || sdate.trim().equals("")){
			sdate="0000-00-00";
		}
		String edate = request.getParameter("edate");//毛利统计日期
		...
		try {
			...
			String[] titlename={"维保分部","维保站","维保合同号","甲方单位名称","统计开始日期","统计结束日期",
					"计提维保费收入","工资支出","年终奖支出","社保支出","报账支出","毛利","毛利率(%)"};
			String[] titleid={"comname","storagename","mcontractno","companyid","jtsdate","jtedate",
					"jtwbfsr","gzzc","nzjzc","sbzc","bzzc","maoli","maoliv"};
			
			String hql="exec sp_ContractGrossMarginReport '"+maintdivision.trim()+"','"+maintstation.trim()+"',"
					+ "'"+contractno.trim()+"','"+salescontractno.trim()+"','"+sdate.trim()+"','"+edate.trim()+"'";
			//System.out.println("维保合同毛利计算报表>>>>"+hql);
            ...
            while(rs.next()){
            	HashMap map= new HashMap();
            	 ...
            	 String recode=rs.getString("recode");//返回代码
            	 String reinfo=rs.getString("reinfo");//返回信息
            	
            	 if("1".equals(recode)){
            		 map.put("reinfo", reinfo);
            		 codeList.add(map);
            	 }else{
           ...
           // hmap.put("totalstr", "平均得分："+df.format(pjdf));
            ...
            if (dform.get("genReport") != null && dform.get("genReport").equals("Y")) {
            	//导出excel
    			response = toExcelRecord(response,reportList,hmap,codeList);
    			...
    			forward = mapping.findForward("contractGrossMarginReportList");	
    		}
		}catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public HttpServletResponse toExcelRecord(HttpServletResponse response,
			List ReportList,HashMap hmap,List codeList) throws IOException {
		...
		//是否显示错误信息
		if(codeList.size()>0){
        	titlename=new String[]{"提示内容"};
    		titleid=new String[]{"reinfo"};
    		ReportList=codeList;
        }

		XSSFWorkbook wb = new XSSFWorkbook();
        XSSFSheet sheet = wb.createSheet("维保合同毛利计算报表");
		...
		if (ReportList != null && ReportList.size()>0) {
			...
			for (int j = 0; j < ReportList.size(); j++) {
				...
				for(int c=0;c<titleid.length;c++){
				    ...
				    if(titleid[c].equals("jtwbfsr") || titleid[c].equals("gzzc")
				    		|| titleid[c].equals("nzjzc") || titleid[c].equals("sbzc")
				    		|| titleid[c].equals("bzzc") || titleid[c].equals("maoli")
				    		|| titleid[c].equals("maoliv")){
...
//		cell1.setCellValue((String)hmap.get("totalstr"));
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("维保合同毛利计算报表", "utf-8") + ".xlsx");
