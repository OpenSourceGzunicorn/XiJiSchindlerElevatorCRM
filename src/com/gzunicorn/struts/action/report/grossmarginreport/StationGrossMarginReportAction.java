 ...
 *
 */
public class StationGrossMarginReportAction extends DispatchAction {

	Log log = LogFactory.getLog(StationGrossMarginReportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "stationgrossmarginreport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 维保站毛利计算报表 >> 查询");
		...
		forward = mapping.findForward("stationGrossMarginReportSearch");		
		return forward;
	}
	...
	public ActionForward toSearchResults(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 维保站毛利计算报表 >> 查询结果");
		ActionForward forward = null;
		HttpSession session = request.getSession();
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session.getAttribute(SysConfig.LOGIN_USER_INFO);
		DynaActionForm dform = (DynaActionForm) form;
		...
		try {
			...
			String[] titlename={"维保分部","维保站","计提维保费收入","工资支出","年终奖支出","社保支出","报账支出","毛利","毛利率(%)"};
			String[] titleid={"comname","storagename","jtwbfsr","gzzc","nzjzc","sbzc","bzzc","maoli","maoliv"};
			
			String hql="exec sp_StationGrossMarginReport '"+maintdivision.trim()+"','"+maintstation.trim()+"','"+sdate.trim()+"','"+edate.trim()+"'";
			//System.out.println("维保站毛利计算报表>>>>"+hql);
           ...
           // hmap.put("totalstr", "平均得分："+df.format(pjdf));
            ...
            if (dform.get("genReport") != null && dform.get("genReport").equals("Y")) {
    			...
    			forward = mapping.findForward("stationGrossMarginReportList");	
    		}
		}catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public HttpServletResponse toExcelRecord(HttpServletResponse response,
        ...
        XSSFSheet sheet = wb.createSheet("维保站毛利计算报表");
...
//		cell1.setCellValue((String)hmap.get("totalstr"));
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("维保站毛利计算报表", "utf-8") + ".xlsx");
