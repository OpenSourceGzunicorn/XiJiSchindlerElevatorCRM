 ...
 *
 */
public class QcwtxReportAction extends DispatchAction {

	Log log = LogFactory.getLog(QcwtxReportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "qcwtxreport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 维保质量问题项报表 >> 查询");
		...
		forward = mapping.findForward("qcwtxReportSearch");		
		return forward;
	}
	...
	public ActionForward toSearchResults(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 维保质量问题项报表 >> 查询结果");
		ActionForward forward = null;
		HttpSession session = request.getSession();
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session.getAttribute(SysConfig.LOGIN_USER_INFO);
		DynaActionForm dform = (DynaActionForm) form;
		...
		try {
			...
			String[] titlename={"问题大项名称","问题小项名称","出现次数","占比(%)"};
			String[] titleid={"msName","DetailName","cxnum","zbi"};

			String hql="exec sp_QcwtxReport '"+maintdivision.trim()+"','"+maintstation.trim()+"','"+maintpersonnel.trim()+"',"
					+ "'"+salescontractno.trim()+"','"+elevatorno.trim()+"','"+sdate.trim()+"','"+edate.trim()+"'";
			//System.out.println("维保质量问题项报表>>>>"+hql);
           ...
           // hmap.put("totalstr", "平均得分："+df.format(pjdf));
            ...
            if (dform.get("genReport") != null && dform.get("genReport").equals("Y")) {
    			...
    			forward = mapping.findForward("qcwtxReportList");	
    		}
		}catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public HttpServletResponse toExcelRecord(HttpServletResponse response,
        ...
        XSSFSheet sheet = wb.createSheet("维保质量问题项报表");
		...
		if (ReportList != null && ReportList.size()>0) {
			...
			for (int j = 0; j < ReportList.size(); j++) {
				...
				for(int c=0;c<titleid.length;c++){
				    ...
				    if(titleid[c].equals("cxnum") || titleid[c].equals("zbi")){
...
//		cell1.setCellValue((String)hmap.get("totalstr"));
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("维保质量问题项报表", "utf-8") + ".xlsx");
