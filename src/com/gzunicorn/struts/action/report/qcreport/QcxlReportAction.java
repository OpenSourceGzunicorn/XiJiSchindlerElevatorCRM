 ...
 *
 */
public class QcxlReportAction extends DispatchAction {

	Log log = LogFactory.getLog(QcxlReportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "qcxlreport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 督查员工作效率报表 >> 查询");
		...
		forward = mapping.findForward("qcxlReportSearch");		
		return forward;
	}
	...
	public ActionForward toSearchResults(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 督查员工作效率报表 >> 查询结果");
		ActionForward forward = null;
		HttpSession session = request.getSession();
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session.getAttribute(SysConfig.LOGIN_USER_INFO);
		DynaActionForm dform = (DynaActionForm) form;

		String checkspeople = request.getParameter("checkspeople");//督查员
		String sdate = request.getParameter("sdate");//派工日期
		if(sdate==null || sdate.trim().equals("")){
			sdate="0000-00-00";
		}
		String edate = request.getParameter("edate");//派工日期
		if(edate==null || edate.trim().equals("")){
			edate="9999-99-99";
		}

		HashMap rmap= new HashMap();			
		rmap.put("checkspeople", checkspeople);
		...
		try {
			...
			String[] titlename={"督查员","派工台数","实际完成台数","不合格台数","完成率(%)","不合格比例(%)"};
			String[] titleid={"SuperviseName","opernum","sjwcnum","bhgnum","wcl","bhgl"};

			String hql="exec sp_QcxlReport '"+checkspeople.trim()+"','"+sdate.trim()+"','"+edate.trim()+"'";
			//System.out.println("督查员工作效率报表>>>>"+hql);
            ...
            if (dform.get("genReport") != null && dform.get("genReport").equals("Y")) {
    			...
    			forward = mapping.findForward("qcxlReportList");	
    		}
		}catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public HttpServletResponse toExcelRecord(HttpServletResponse response,
        ...
        XSSFSheet sheet = wb.createSheet("督查员工作效率报表");
		...
		if (ReportList != null && ReportList.size()>0) {
			...
			for (int j = 0; j < ReportList.size(); j++) {
				...
				for(int c=0;c<titleid.length;c++){
				    ...
				    if(titleid[c].equals("SuperviseName")){
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("督查员工作效率报表", "utf-8") + ".xlsx");
