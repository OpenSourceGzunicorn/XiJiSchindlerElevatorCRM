 ...
 *
 */
public class QczdReportAction extends DispatchAction {

	Log log = LogFactory.getLog(QczdReportAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		/** **********开始用户权限过滤*********** */
		SysRightsUtil.filterModuleRight(request, response,SysRightsUtil.NODE_ID_FORWARD + "qczdreport", null);
		/** **********结束用户权限过滤*********** */
	...
	public ActionForward toSearchRecord(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 重大不合格项报表 >> 查询");
		...
		forward = mapping.findForward("qczdReportSearch");		
		return forward;
	}
	...
	public ActionForward toSearchResults(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("navigator.location", " 重大不合格项报表 >> 查询结果");
		ActionForward forward = null;
		HttpSession session = request.getSession();
		ViewLoginUserInfo userInfo = (ViewLoginUserInfo) session.getAttribute(SysConfig.LOGIN_USER_INFO);
		DynaActionForm dform = (DynaActionForm) form;
		...
		try {
			...
			String[] titlename={"维保分部","维保站","参与维保工","督查员","销售合同号","电梯编号",
					"督查结果提交日期","重大不合格大项名称","小项名称","备注"};
			String[] titleid={"ComName","StorageName","r5name","ChecksPeopleName","SalesContractNo","ElevatorNo",
					"ChecksDateTime","msName","DetailName","rem"};
			
			String hql="exec sp_QczdReport '"+maintdivision.trim()+"','"+maintstation.trim()+"','"+maintpersonnel.trim()+"',"
					+ "'"+salescontractno.trim()+"','"+elevatorno.trim()+"','"+sdate.trim()+"','"+edate.trim()+"'";
			//System.out.println("重大不合格项报表>>>>"+hql);
           ...
           // hmap.put("totalstr", "平均得分："+df.format(pjdf));
            ...
            if (dform.get("genReport") != null && dform.get("genReport").equals("Y")) {
    			...
    			forward = mapping.findForward("qczdReportList");	
    		}
		}catch(Exception e) {
			e.printStackTrace();
		} finally {
	...
	public HttpServletResponse toExcelRecord(HttpServletResponse response,
        ...
        XSSFSheet sheet = wb.createSheet("重大不合格项报表");
...
//		cell1.setCellValue((String)hmap.get("totalstr"));
		...
		response.setHeader("Content-disposition", "offline; filename="+URLEncoder.encode("重大不合格项报表", "utf-8") + ".xlsx");
